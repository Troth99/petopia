...
import { getAllHomeRabbits, getAllMiniRabbits } from "../../services/rabbitsRequester.js";

export default async function showALlMiniRabbits() {
    const miniRabbits = await getAllMiniRabbits();
    render(miniRabbitsTemplate(miniRabbits), mainEl);
}

function miniRabbitsTemplate(exoticCats) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${exoticCats.length === 0
            ? html`<p class="no-data">Няма налични мини зайчета за осиновяване в момента. </p>`
            : exoticCats.map(
                (breed) => html`
