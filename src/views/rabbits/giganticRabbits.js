...
import { getAllGiganticRabbits, getAllHomeRabbits } from "../../services/rabbitsRequester.js";

export default async function showALlGiganticRabbits() {
    const giganticRabbits = await getAllGiganticRabbits();
    render(giganticRabbitsTemplate(giganticRabbits), mainEl);
}

function giganticRabbitsTemplate(exoticCats) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Гигантски зайчета</h2>

            ${exoticCats.length === 0
            ? html`<p class="no-data">Няма налични Гигантски зайчета за осиновяване в момента. </p>`
            : exoticCats.map(
                (breed) => html`
