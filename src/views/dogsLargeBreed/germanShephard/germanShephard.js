...
import { getAllGermanShepherd } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showAllGermanShepherd() {
    const germanShepherd = await getAllGermanShepherd();
    render(germanShepherdTemplate(germanShepherd), mainEl);
}

function germanShepherdTemplate(germanShepherd) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${germanShepherd.length === 0
            ? html`<p class="no-data">Няма налични Немски овчарки за осиновяване в момента. </p>`
            : germanShepherd.map(
                (breed) => html`
