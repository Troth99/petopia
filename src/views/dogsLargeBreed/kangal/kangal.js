...
import {  getALlKangal, getAllkavkazshepherd, getAllKingDogs, getALlnewfoundland, getAllSaintBernard } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showKangalView() {
    const kangal = await getALlKangal();
    render(kangalTemplate(kangal), mainEl);
}

function kangalTemplate(kangal) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${kangal.length === 0
            ? html`<p class="no-data">Няма налични Кангал за осиновяване в момента. </p>`
            : kangal.map(
                (breed) => html`
