...
import {  getAllkavkazshepherd, getALlnewfoundland, getAllSaintBernard } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function kavkazShepherdView() {
    const kavkazShepherd = await getAllkavkazshepherd();
    render(kavkazShepherdTemplate(kavkazShepherd), mainEl);
}

function kavkazShepherdTemplate(kavkazShepherd) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Кавказка овчарка</h2>

            ${kavkazShepherd.length === 0
            ? html`<p class="no-data">Няма налични Кавказка овчарка за осиновяване в момента. </p>`
            : kavkazShepherd.map(
                (breed) => html`
