...
import {  getALlnewfoundland, getAllSaintBernard } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function newfoundlandView() {
    const newfoundland = await getALlnewfoundland();
    render(newfoundlandTemplate(newfoundland), mainEl);
}

function newfoundlandTemplate(newFoundland) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${newFoundland.length === 0
            ? html`<p class="no-data">Няма налични Нюфаундленди за осиновяване в момента. </p>`
            : newFoundland.map(
                (breed) => html`
