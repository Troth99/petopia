...
import {getAllGoldenRetriever } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showALlGoldenRetriever() {
    const goldenRetriever = await getAllGoldenRetriever();
    render(goldenRetrieverTemplate(goldenRetriever), mainEl);
}

function goldenRetrieverTemplate(goldenRetriever) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${goldenRetriever.length === 0
            ? html`<p class="no-data">Няма налични Голдън Ретривъри за осиновяване в момента. </p>`
            : goldenRetriever.map(
                (breed) => html`
