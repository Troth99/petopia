...
import {getAllLabrador } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showAllLabradors() {
    const labrador = await getAllLabrador();
    render(labradorTemplate(labrador), mainEl);
}

function labradorTemplate(labrador) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${labrador.length === 0
            ? html`<p class="no-data">Няма налични Лабрадор за осиновяване в момента. </p>`
            : labrador.map(
                (breed) => html`
