...
import { getAllShortCoatBreeds } from "../../services/breeds";
import { auth } from "../../config/firebaseInit.js";

export default async function showShortHairCoatView() {
    const shortCoatDogs = await getAllShortCoatBreeds();
    console.log(shortCoatDogs)
    render(shortCoatTemplate(shortCoatDogs), mainEl);
}

function shortCoatTemplate(shortHairDogs) {
...
<section class="short-hair-wrapper">
    <h2 class="short-hair-title">Късокосмести Кучета</h2>

    <div class="short-hair-section">
        ${shortHairDogs.length === 0
            ? html`<p class="short-hair-empty">Няма налични кучета с къса козина в момента.</p>`
            : shortHairDogs.map(
                (breed) => html`
