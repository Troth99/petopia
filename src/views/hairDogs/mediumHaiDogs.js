...
import { getAllLongHairCoatDogs, getAllMediumHairDogs, getAllShortCoatBreeds } from "../../services/breeds";
import { auth } from "../../config/firebaseInit.js";

export default async function showMediumHairDogsView() {
    const mediumCoatDogs = await getAllMediumHairDogs();
    render(mediumCoatDogTemplate(mediumCoatDogs), mainEl);
}

function mediumCoatDogTemplate(mediumCoatDogs) {
...
<section class="short-hair-wrapper">
    <h2 class="short-hair-title">Средна козина Кучета</h2>

    <div class="short-hair-section">
        ${mediumCoatDogs.length === 0
            ? html`<p class="short-hair-empty">Няма налични кучета със средна козина в момента.</p>`
            : mediumCoatDogs.map(
                (breed) => html`
