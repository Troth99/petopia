...
import {getAllInteligentCats } from "../../services/catsBreedRequester.js";

export default async function showAllInteligentCats() {
    const intelligentCats = await getAllInteligentCats();
    render(intelligentCatsTemplate(intelligentCats), mainEl);
}

function intelligentCatsTemplate(homeCats) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Интелигентни котки</h2>

            ${homeCats.length === 0
            ? html`<p class="no-data">Няма налични Интелигентни котки за осиновяване в момента. </p>`
            : homeCats.map(
                (breed) => html`
