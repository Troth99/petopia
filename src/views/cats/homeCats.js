...
import { getAllHomeCats } from "../../services/catsBreedRequester.js";

export default async function showALlHomeCats() {
    const homeCats = await getAllHomeCats();
    render(homeCatsTemplate(homeCats), mainEl);
}

function homeCatsTemplate(homeCats) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${homeCats.length === 0
            ? html`<p class="no-data">Няма налични домашни котки за осиновяване в момента. </p>`
            : homeCats.map(
                (breed) => html`
