...
import { getAllHomeCats, getAllLongHairCats } from "../../services/catsBreedRequester.js";

export default async function showAllLongHairCats() {
    const longHairCats = await getAllLongHairCats();
    console.log(longHairCats)
    render(longHairTemplateCats(longHairCats), mainEl);
}

function longHairTemplateCats(homeCats) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Котки с дълга козина</h2>

            ${homeCats.length === 0
            ? html`<p class="no-data">Няма налични котки с дълга козина за осиновяване в момента. </p>`
            : homeCats.map(
                (breed) => html`
