...
import { getAllHomeCats, getAllhypoallergenicCats } from "../../services/catsBreedRequester.js";

export default async function showAllHypoalegenicCats() {
    const hypoallergenicCats = await getAllhypoallergenicCats();
    render(hypoallergenicTemplateCats(hypoallergenicCats), mainEl);
}

function hypoallergenicTemplateCats(hypoallergenicCats) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Хипоалергенни котки</h2>

            ${hypoallergenicCats.length === 0
            ? html`<p class="no-data">Няма налични Хипоалергенни котки за осиновяване в момента. </p>`
            : hypoallergenicCats.map(
                (breed) => html`
