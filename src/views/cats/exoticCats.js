...
import { getAllExoticCats} from "../../services/catsBreedRequester.js";

export default async function showAllExoticCats() {
    const exoticCats = await getAllExoticCats();
    render(exoticCatsTemplate(exoticCats), mainEl);
}

function exoticCatsTemplate(exoticCats) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Екзотични котки</h2>

            ${exoticCats.length === 0
            ? html`<p class="no-data">Няма налични Екзотични котки за осиновяване в момента. </p>`
            : exoticCats.map(
                (breed) => html`
