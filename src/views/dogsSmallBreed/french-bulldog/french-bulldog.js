...
import { getAllFrenchBuldog, getAllYokrshireTerier } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js"; 
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showFrenchBuldog() {
    const frenchBuldog = await getAllFrenchBuldog();
    render(frenchBuldogTemplate(frenchBuldog), mainEl);
}

function frenchBuldogTemplate(frenchBuldog) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Френски булдог</h2>

            ${frenchBuldog.length === 0
                ? html`<p class="no-data">Няма налични Френски булдог за осиновяване в момента. </p>`
                : frenchBuldog.map(
                      (breed) => html`
