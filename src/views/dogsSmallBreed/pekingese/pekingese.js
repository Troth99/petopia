...
import { getAllPekinese, getAllYokrshireTerier } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js"; 
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showPekineseView() {
    const pekines = await getAllPekinese();
    render(pekineseTemplate(pekines), mainEl);
}

function pekineseTemplate(pekines) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${pekines.length === 0
                ? html`<p class="no-data">Няма налични Пекинези за осиновяване в момента. </p>`
                : pekines.map(
                      (breed) => html`
