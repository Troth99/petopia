...
import { getAllAustiranShepherd, getAllBeagles, getAllBirthdaySpanel, getAllBullTeriers, getAllEnglishCockerSpaniel, getAllShetlandSheepdog } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showBrittanySpanel() {
    const brittanySpanel = await getAllBirthdaySpanel();
    render(brittanySpanelTemplate(brittanySpanel), mainEl);
}

function brittanySpanelTemplate(brittanySpanel) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Британи шпаньол</h2>

            ${brittanySpanel.length === 0
            ? html`<p class="no-data">Няма налични Британи шпаньол за осиновяване в момента. </p>`
            : brittanySpanel.map(
                (breed) => html`
