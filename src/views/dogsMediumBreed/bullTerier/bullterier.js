...
import { getAllAustiranShepherd, getAllBeagles, getAllBullTeriers, getAllEnglishCockerSpaniel } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showBullTerirerView() {
    const bullTerier = await getAllBullTeriers();
    render(bullTerierTemplate(bullTerier), mainEl);
}

function bullTerierTemplate(bullTerier) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${bullTerier.length === 0
            ? html`<p class="no-data">Няма налични Бультериѐри за осиновяване в момента. </p>`
            : bullTerier.map(
                (breed) => html`
