...
import { getAllAustiranShepherd, getAllBeagles, getAllBullTeriers, getAllChowChow, getAllEnglishCockerSpaniel, getAllShetlandSheepdog } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showChowChowView() {
    const chowChow = await getAllChowChow();
    render(chowChowTemplate(chowChow), mainEl);
}

function chowChowTemplate(chowChow) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${chowChow.length === 0
            ? html`<p class="no-data">Няма налични Чао-Чао за осиновяване в момента. </p>`
            : chowChow.map(
                (breed) => html`
