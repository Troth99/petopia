...
import { getAllAustiranShepherd, getAllBeagles, getAllBullTeriers, getAllEnglishCockerSpaniel, getAllNorwegianElkhoud, getAllShetlandSheepdog } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showNorvegianElkhoundView() {
    const norvegianElhound = await getAllNorwegianElkhoud();
    render(norvegianElhoundTemplate(norvegianElhound), mainEl);
}

function norvegianElhoundTemplate(norvegianElkhound) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Норвежки елкхунд</h2>

            ${norvegianElkhound.length === 0
            ? html`<p class="no-data">Няма налични Норвежки елкхунд за осиновяване в момента. </p>`
            : norvegianElkhound.map(
                (breed) => html`
