...
import { getAllAustiranShepherd, getAllBeagles, getAllEnglishCockerSpaniel } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showAustralianShepherdView() {
    const australianShepherd = await getAllAustiranShepherd();
    render(australianShepherdTemplate(australianShepherd), mainEl);
}

function australianShepherdTemplate(beagle) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Австралийска овчарка</h2>

            ${beagle.length === 0
            ? html`<p class="no-data">Няма налични Австралийска овчарки за осиновяване в момента. </p>`
            : beagle.map(
                (breed) => html`
