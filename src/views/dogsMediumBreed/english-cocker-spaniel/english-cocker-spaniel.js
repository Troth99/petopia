...
import { getAllEnglishCockerSpaniel } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showEnglishCckerSpanielView() {
    const englishCockerSpaniel = await getAllEnglishCockerSpaniel();
    render(englishCockerSpaniolTempalte(englishCockerSpaniel), mainEl);
}

function englishCockerSpaniolTempalte(kingCharlsDog) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Английски кокер шпаньол</h2>

            ${kingCharlsDog.length === 0
            ? html`<p class="no-data">Няма налични Английски кокер шпаньол за осиновяване в момента. </p>`
            : kingCharlsDog.map(
                (breed) => html`
