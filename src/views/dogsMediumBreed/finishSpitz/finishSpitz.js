...
import { getAllAustiranShepherd, getAllBeagles, getAllBullTeriers, getAllChowChow, getAllEnglishCockerSpaniel, getAllFinishSpitz, getAllShetlandSheepdog } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showFinishSpitzView() {
    const finishSpitz = await getAllFinishSpitz();
    render(finishSpitzTemplate(finishSpitz), mainEl);
}

function finishSpitzTemplate(finishSpitz) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Финландско шпице</h2>

            ${finishSpitz.length === 0
            ? html`<p class="no-data">Няма налични Финландско шпице за осиновяване в момента. </p>`
            : finishSpitz.map(
                (breed) => html`
