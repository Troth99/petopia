...
import { getAllAustiranShepherd, getAllBeagles, getAllBullTeriers, getAllEnglishCockerSpaniel, getAllShetlandSheepdog } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showShetlandSheperd() {
    const shetlandSheperd = await getAllShetlandSheepdog();
    render(shetlandSheperdTemplate(shetlandSheperd), mainEl);
}

function shetlandSheperdTemplate(shetlandShepered) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Шетландска овчарка</h2>

            ${shetlandShepered.length === 0
            ? html`<p class="no-data">Няма налични Шетландски овчарки за осиновяване в момента. </p>`
            : shetlandShepered.map(
                (breed) => html`
