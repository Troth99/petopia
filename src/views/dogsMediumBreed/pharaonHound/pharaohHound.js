...
import { getAllAustiranShepherd, getAllBeagles, getAllBullTeriers, getAllChowChow, getAllEnglishCockerSpaniel, getAllFinishSpitz, getAllPHaraonDog, getAllShetlandSheepdog } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showPharaonView() {
    const PharaonHound = await getAllPHaraonDog();
    render(pharaonTemplate(PharaonHound), mainEl);
}

function pharaonTemplate(pharaon) {
    ...
    return html`
        <section class="chihuahua-info">
            <h2>Фараоново куче</h2>

            ${pharaon.length === 0
            ? html`<p class="no-data">Няма налични Фараоново куче за осиновяване в момента. </p>`
            : pharaon.map(
                (breed) => html`
