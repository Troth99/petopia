...
import { getAllBeagles, getAllEnglishCockerSpaniel } from "../../../services/breeds.js";
import { auth } from "../../../config/firebaseInit.js";
import { doc, deleteDoc } from "firebase/firestore";
import { db } from "../../../config/firebaseInit.js";

export default async function showBeagleView() {
    const beagle = await getAllBeagles();
    render(beagleTemplate(beagle), mainEl);
}

function beagleTemplate(beagle) {
    ...
    return html`
        <section class="chihuahua-info">
            ...
            ${beagle.length === 0
            ? html`<p class="no-data">Няма налични Бийгъли за осиновяване в момента. </p>`
            : beagle.map(
                (breed) => html`
